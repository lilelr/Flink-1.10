    ...
    optimizerBuilder
      .withLeaseRejectAction(new Action1[VirtualMachineLease]() {
        def call(lease: VirtualMachineLease) {
          ...
          LOG.error(s"Declined offer ${lease.getId} from ${lease.hostname()} "
...
object LaunchCoordinator {
  ...
  private def processAssignments(
    ...
    log.info("lele in processAssignments and  offer.getResources")
    val resources =
      assignments.getLeasesUsed.asScala.flatMap(_.asInstanceOf[Offer].getResources.asScala)

//    assignments.getLeasesUsed.get(0).
//    val allocation = new MesosResourceAllocation(resources.asJava)

    val allocation = new MesosResourceAllocation(resources.asJava,assignments.getLeasesUsed.get(0).getAttributeMap())
    log.info(s"Assigning resources: ${Utils.toString(allocation.getRemaining)}")

    def taskInfo(assignment: TaskAssignmentResult): Protos.TaskInfo = {
      log.info(s"Processing task ${assignment.getTaskId}")
